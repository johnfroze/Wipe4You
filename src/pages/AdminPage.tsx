...
export function AdminPage({ members, onMembersChange }: Props) {
  ...
  const handleAddDkp = async (id: string, current: number) => {
    ...
    setEditingDkp((prev) => ({ ...prev, [id]: false }));
    setDkpInputs((prev) => ({ ...prev, [id]: '' }));
  ...
  const handleChangeRole = async (id: string, current: Member['role']) => {
    const roles: Member['role'][] = ['member', 'elder', 'leader'];
  ...
  const handleRename = async (id: string, current: string) => {
    const name = prompt('New username:', current);
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage guild members and DKP</p>
        </div>
          ...
          {filtered.map((m) => (
            ...
            >
              <div className="flex items-center gap-4">
                <img src={m.avatar} alt="" className="w-12 h-12 rounded-full border border-[#333]" />
                <div>
                  <div className="font-medium flex items-center gap-2">
                    {m.username}
                    <button
                      onClick={() => handleRename(m.id, m.username)}
                  ...
                  >
                    ...
                    <span className="text-gray-600 ml-1">(click to cycle)</span>
                  </button>
                </div>
              </div>
                ...
                {editingDkp[m.id] ? (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setDkpInputs((prev) => ({ ...prev, [m.id]: e.target.value }))
                      }
                      type="number"
                      placeholder="Amount"
                    ...
                    />
                    <button
                      onClick={() => handleAddDkp(m.id, m.dkp)}
                    ...
                    <button
                      onClick={() => handleRemoveDkp(m.id, m.dkp)}
                    ...
                    <button
                      onClick={() => setEditingDkp((prev) => ({ ...prev, [m.id]: false }))}
                  ...
                  <>
                    <div className="text-cyan-400 font-bold tabular-nums mr-2">{m.dkp} DKP</div>
                    <button
                      onClick={() => setEditingDkp((prev) => ({ ...prev, [m.id]: true }))}
